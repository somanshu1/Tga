...
full_data, node_features, edge_type, train_data, val_data, test_data = \
    get_data_node_classification(DATA, use_validation=args.use_validation)

max_idx = max(full_data.unique_nodes)

train_ngh_finder = get_neighbor_finder(full_data, uniform=UNIFORM, max_node_idx=max_idx)  # train_data
...
for i in range(args.n_runs):
    ...
    beat_test_auc = eval_node_classification(tgn, decoder, test_data, full_data.edge_idxs, BATCH_SIZE,
                                            n_neighbors=NUM_NEIGHBORS)
    # else:
    ...
    logger.info(f'Run {i}, Bset val auc: {best_val_auc}, test auc: {beat_test_auc}')
    final_val_aucs.append(best_val_auc)
    final_test_aucs.append(beat_test_auc)

logger.info(f'Final val auc: {np.mean(final_val_aucs)} ± {np.std(final_val_aucs)}')
logger.info(f'Final test auc: {np.mean(final_test_aucs)} ± {np.std(final_test_aucs)}')
